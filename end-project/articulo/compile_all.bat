...
set MIKTEX_BIN="C:\Users\Keiner Cano\AppData\Local\Programs\MiKTeX\miktex\bin\x64"

echo.
echo === Compilando IEEE ===
%MIKTEX_BIN%\xelatex -shell-escape -output-directory=build main_ieee.tex
if %ERRORLEVEL% neq 0 (
    echo Error compilando IEEE - primera pasada
    goto :error
)

%MIKTEX_BIN%\biber build/main_ieee.bcf
if %ERRORLEVEL% neq 0 (
...
echo.
echo === Compilando ACM ===
%MIKTEX_BIN%\xelatex -shell-escape -output-directory=build main_acm.tex
if %ERRORLEVEL% neq 0 (
    echo Error compilando ACM - primera pasada
    goto :error
)

%MIKTEX_BIN%\bibtex build/main_acm.aux
if %ERRORLEVEL% neq 0 (
...
%MIKTEX_BIN%\xelatex -shell-escape -output-directory=build main_apa7.tex
if %ERRORLEVEL% neq 0 (
    echo Error compilando APA7 - primera pasada
    goto :error
)

%MIKTEX_BIN%\biber build/main_apa7.bcf
if %ERRORLEVEL% neq 0 (
